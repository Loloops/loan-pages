...
window.addEventListener('DOMContentLoaded', function () {
  ...
  feedSlider.init();
  var videoPlayer = new _modules_videoPlayer__WEBPACK_IMPORTED_MODULE_0__["default"]('.showup .play', '.overlay');
  videoPlayer.init();
...
function () {
  ...
  _createClass(Difference, [{
    key: "bindTriggers",
    value: function bindTriggers(_ref) {
      ...
      selector.querySelector('.plus').addEventListener('click', function () {
        if (counter !== item.length - 2) {
          ...
          //если перебрали со счетчиком скрываем кнопку для добавления
          item[counter].style.display = 'flex';
    ...
    key: "hideItems",
    value: function hideItems(_ref2) {
      var item = _ref2.item;
      item.forEach(function (item, i, arr) {
        if (i !== arr.length - 1) {
          //номер по порядку не является последним элементом в коллекции
  ...
  _createClass(MainSlider, [{
    ...
    value: function showSlides(n) {
      ...
      this.slides.forEach(function (slide) {
        slide.style.display = 'none'; // slide.classList.add('animated', 'fadeInUp')
...
function (_Slider) {
  ...
  _createClass(MiniSlider, [{
    key: "decoraizeSlides",
    value: function decoraizeSlides() {
      var _this = this;
